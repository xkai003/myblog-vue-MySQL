...
export default {
  ...
  methods: {
    ...
    async submit () {
      ...
      try {
        const res = await axios.post('http://47.79.40.65:3006/works', newWorks)
        if (res.status === 201) {
          alert('添加作品成功！')
          // 重新刷新页面
          location.reload()
    ...
    async del (id) {
      ...
      if (confirmed) {
        try {
          const apiUrl = `http://47.79.40.65:3006/works/${id}`
    ...
    async editsubmit() {
      ...
      try {
        ...
        const apiUrl = `http://47.79.40.65:3006/works/${this.selectedId}` // 替换为你的更新 API
  ...
  async created() {
    try {
      ...
      if (res.data && res.data.works) {  // 确保 res.data.works 存在
        this.worklist = res.data.works.sort((a, b) => {
        ...
        console.warn("后端返回的数据格式不正确，缺少 'works' 字段");
      }
    } catch (error) {
...
.food .workstable::-webkit-scrollbar{
  display: none;
}

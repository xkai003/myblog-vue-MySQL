  ...
  <div class="box">
    <div class="center">
      <div class="form">
        ...
        <div class="image">
          <span>图片：</span>
          <input type="text" placeholder="请输入图片(不超过300字)" v-model="image">
        </div>
        <div class="description">
          <span>描述：</span>
          <textarea placeholder="请输入描述(不超过300字)" v-model="description"></textarea>
        </div>
        <div class="btn">
...
export default {
  ...
  methods: {
    ...
    async submit () {
      ...
      try {
        const res = await axios.post('http://47.79.40.65:3006/travel', newtravel)
        if (res.status === 201) {
          alert('添加旅游成功！')
          // 重新刷新页面
          location.reload()
    ...
    async del (id) {
      ...
      if (confirmed) {
        try {
          const apiUrl = `http://47.79.40.65:3006/travel/${id}`
    ...
    async editsubmit() {
      ...
      try {
        ...
        const apiUrl = `http://47.79.40.65:3006/travel/${this.selectedId}` // 替换为你的更新 API
  ...
  async created() {
    try {
      ...
      if (res.data && res.data.travel) {  // 确保 res.data.travel 存在
        this.travellist = res.data.travel.sort((a, b) => {
        ...
        console.warn("后端返回的数据格式不正确，缺少 'travel' 字段");
      }
    } catch (error) {
...
.food .travetable::-webkit-scrollbar{
  display: none;
}
.food .travellist ul{
  display: flex;

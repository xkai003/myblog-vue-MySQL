  ...
  <div class="box">
    <div class="center">
      <div class="form">
        ...
        <div class="label">
          <span>链接：</span>
          <input type="text" placeholder="请输入链接(不超过300字)" v-model="label">
        </div>
        <div class="technology">
          <span>技术栈：</span>
          <textarea placeholder="请输入技术栈(不超过300字)" v-model="technology"></textarea>
        </div>
        <div class="btn">
...
export default {
  ...
  methods: {
    ...
    async submit () {
      ...
      try {
        const res = await axios.post('http://47.79.40.65:3006/notes', newNotes)
        if (res.status === 201) {
          alert('添加笔记成功！')
          // 重新刷新页面
          location.reload()
    ...
    async del (id) {
      ...
      if (confirmed) {
        try {
          const apiUrl = `http://47.79.40.65:3006/notes/${id}`
    ...
    async editsubmit() {
      ...
      try {
        ...
        const apiUrl = `http://47.79.40.65:3006/notes/${this.selectedId}` // 替换为你的更新 API
  ...
  async created() {
    try {
      ...
      if (res.data && res.data.notes) {  // 确保 res.data.notes 存在
        this.noteslist = res.data.notes.sort((a, b) => {
        ...
        console.warn("后端返回的数据格式不正确，缺少 'notes' 字段");
      }
    } catch (error) {
...
.food .notestable::-webkit-scrollbar{
  display: none;
}
.food .noteslist ul{
  display: flex;
